...
**Gen4 SSDs are fully backward compatible with Gen2/Gen3 hosts.** PCIe mandates backward compatibility — during link training, the SSD and host negotiate the highest mutually supported generation. A Gen4 SSD (e.g. 990 Pro 2TB) plugged into our Gen3 x4 PL-side XDMA trains at Gen3 x4 (~4 GB/s ceiling); plugged into the Gen2 x4 PS-side hardened PCIe, it trains at Gen2 x4 (~2 GB/s ceiling). The NVMe register interface, BAR layout, command set, and M.2 M-key physical connector are all identical across generations. Since the 990 Pro's sustained post-cache write (~2.25 GB/s) is below the Gen3 x4 link ceiling, the PL path delivers full SSD speed with no penalty. On the PS Gen2 x4 path, the link becomes a slight bottleneck (~1.5–1.8 GB/s practical vs 2.25 GB/s SSD capability).
---
